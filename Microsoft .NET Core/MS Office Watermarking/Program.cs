        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (args.Length == 0)
                {
                    ...
                    {
                        Console.WriteLine("Please specify a document to watermark.");
                        Console.ReadKey();
                        return;
                    }
                }
                ...
                byte[] watermarkedFile = client.ApplyWatermarkAsync(sourceFile, openOptions, conversionSettings).GetAwaiter().GetResult();

                // ** Write the watermarked file back to the file system.
                string destinationFileName = Path.Combine(Path.GetDirectoryName(sourceFileName),
                                             Path.GetFileNameWithoutExtension(sourceFileName) + "_Watermarked." + conversionSettings.Format);
